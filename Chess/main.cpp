	...
	for (int row = Board::MIN_ROW_INDEX; row < Board::MAX_ROW_INDEX; row++)
	{
		for (int col = Board::MIN_COL_INDEX; col < Board::MAX_COL_INDEX; col++)
		{
			if (IsValid(board.C_At(col, row)))
			{
				...
				switch (pieceType)
				{
				case PieceType::PAWN: Pawn.Draw(isWhite);
					break;
				case PieceType::ROOK: Rook.Draw(isWhite);
					break;
				case PieceType::KNIGHT: Knight.Draw(isWhite);
					break;
				case PieceType::BISHOP: Bishop.Draw(isWhite);
					break;
				case PieceType::QUEEN: Queen.Draw(isWhite);
					break;
				case PieceType::KING: King.Draw(isWhite);
					break;
				}
				glPopMatrix();
			}
